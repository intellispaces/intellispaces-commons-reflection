...
import intellispaces.javastatements.object.instance.AnnotationElementBuilder;
...
public interface AnnotationFunctions {
  ...
  static <A extends Annotation> A asAnnotation(AnnotationInstance instance, Class<A> annotationClass) {
    if (!annotationClass.getCanonicalName().equals(instance.annotationStatement().canonicalName())) {
      throw new JavaStatementException("Illegal annotation class. Expected {}. Actual {}", annotationClass.getCanonicalName(),
    ...
    return (A) Proxy.newProxyInstance(
        ...
        (proxy, method, methodArgs) -> {
          ...
          if (elementValue.isPresent()) {
            ...
            throw new JavaStatementException("Unsupported method {}", method.getName());
          }
        }
    );
  }
