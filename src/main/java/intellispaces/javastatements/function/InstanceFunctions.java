...
import intellispaces.javastatements.object.instance.AnnotationInstanceBuilder;
import intellispaces.javastatements.object.instance.ArrayInstanceBuilder;
import intellispaces.javastatements.object.instance.ClassInstanceBuilder;
import intellispaces.javastatements.object.instance.EnumInstanceBuilder;
import intellispaces.javastatements.object.instance.PrimitiveInstanceBuilder;
import intellispaces.javastatements.object.instance.StringInstanceBuilder;
...
public interface InstanceFunctions {
  ...
  static Object instanceToObject(Instance instance) {
    if (instance == null) {
      ...
      throw new JavaStatementException("Unsupported instance type {}", instance.getClass().getName());
    }
  }
  ...
  static Instance objectToInstance(Object value, Session session) {
    ...
    if (value instanceof Boolean) {
      ...
      TypeReference typeReference = TypeFunctions.getTypeReference(variableElement.asType(), session);
      ...
      CustomTypeReference typeReference = TypeFunctions.getTypeReference(declaredType, session);
      ...
      throw new JavaStatementException("Unsupported object class {}", value.getClass().getCanonicalName());
    }
    return instance;
  }
  ...
  private static TypeReference arrayItemsType(List<Instance> values) {
    ...
    if (StatementTypes.PrimitiveInstance.equals(value.statementType())) {
      ...
      throw new JavaStatementException("Unsupported array element type in annotation element: " + value.statementType().typename());
    }
  }
  ...
  private static Enum asEnum(EnumInstance instance) {
    try {
      ...
      throw new JavaStatementException(e, "Class by name {} is not found", instance.type().canonicalName());
    }
  }
  ...
  private static Object[] asArray(ArrayInstance instance) {
    Class<?> elementClass = TypeFunctions.getClass(instance.elementType());
  ...
  private static Annotation asAnnotation(AnnotationInstance instance) {
    ...
    Class<?> annotationClass = TypeFunctions.getClass(annotationClassName);
    if (!Annotation.class.isAssignableFrom(annotationClass)) {
      throw new JavaStatementException("Class {} does not extend {}", annotationClassName, Annotation.class.getName());
    }
