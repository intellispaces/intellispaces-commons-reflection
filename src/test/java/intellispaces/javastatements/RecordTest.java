...
/**
 * Tests for {@link intellispaces.javastatements.model.custom.RecordStatement}.
 */
public class RecordTest extends AbstractCustomTypeTest {
  ...
  public void testEmptyRecord() {
    ...
    assertThat(recordStatement.canonicalName()).isEqualTo("intellispaces.javastatements.sample.EmptyRecord");
    ...
    CommonFunctions.handle(recordStatement.annotations().get(0), annInstance -> {
  ...
  public void testRecordImplementedTwoInterfaces() {
    // Given
    final var interface1Name = "intellispaces.javastatements.sample.RecordImplementedTwoInterfaces.Interface1";
    final var interface2Name = "intellispaces.javastatements.sample.RecordImplementedTwoInterfaces.Interface2";
    ...
    assertThat(recordStatement.canonicalName()).isEqualTo("intellispaces.javastatements.sample.RecordImplementedTwoInterfaces.TesteeRecord");

    assertThat(recordStatement.typeParameters()).isEmpty();

    assertThat(recordStatement.implementedInterfaces()).hasSize(2);
    CommonFunctions.handle(recordStatement.implementedInterfaces().get(0), implInterface -> {
      assertThat(implInterface.targetType().canonicalName()).isEqualTo(interface1Name);
      assertThat(implInterface.typeArguments()).isEmpty();
    });
    CommonFunctions.handle(recordStatement.implementedInterfaces().get(1), implInterface -> {
    ...
    CommonFunctions.handle(recordStatement.parentTypes().get(0), parentType -> {
    ...
    CommonFunctions.handle(recordStatement.parentTypes().get(1), parentType -> {
      assertThat(parentType.targetType().canonicalName()).isEqualTo(interface1Name);
      assertThat(parentType.typeArguments()).isEmpty();
    });
    CommonFunctions.handle(recordStatement.parentTypes().get(2), parentType -> {
  ...
  public void testGenericRecordWithCyclicTypeDependencyCase1() {
    ...
    assertThat(typeReference.targetType().asRecord()).isPresent();
    ...
    CommonFunctions.handle(recordStatement.typeParameters().get(0), typeParam -> {
  ...
  public void testGenericRecordWithCyclicTypeDependencyCase2() {
    ...
    assertThat(recordStatement.typeParameters()).hasSize(1);
    CommonFunctions.handle(recordStatement.typeParameters().get(0), classATypeParam -> {
      ...
      CommonFunctions.handle(classATypeParam.extendedBounds().get(0).asCustomTypeReference().orElseThrow().targetType(), classBExtendedBound -> {
        assertThat(classBExtendedBound.canonicalName()).isEqualTo("intellispaces.javastatements.sample.GenericRecordWithCyclicTypeDependencyCase2.RecordB");
  ...
  private void testRecordWithOneMethod(
      String recordName, String methodName, HandlerAction<MethodStatement> methodValidator, List<String> additionalImports
  ) {
    // Given
    String canonicalClassName = "intellispaces.javastatements.sample." + recordName;
